...
const getAllPublicSleepLogs = (req, res, next) => {
  db.any(
    "SELECT users.firebase_id, users.username, users.first_name, users.last_name, sleep_logs.post_date, sleep_logs.remember_dream, sleep_logs.sleep_interrupted, sleep_logs.sleep_start, sleep_logs.sleep_end, sleep_logs.notes FROM sleep_logs JOIN users ON users.firebase_id = sleep_logs.user_id WHERE is_private = FALSE"
  )
    .then((data) => {
      res.status(200).json({
        status: "Success",
